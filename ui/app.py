    ...
    with tab3:
        ...
        if not st.session_state.get("testcases_generated", False):
            ...
            try:
                ...
                # 평균 검증 점수 계산
                avg_scores = {
                    "정확성": validation_df["정확성"].mean(),
                    "완전성": validation_df["완전성"].mean(),
                    "명확성": validation_df["명확성"].mean(),
                    "플랫폼_적합성": validation_df["플랫폼_적합성"].mean(),
                    "총점": validation_df["총점"].mean()
                }
                
                # 점수 시각화
                st.subheader("평균 검증 점수")
                ...
                filename = f"테스트케이스_{timestamp}.xlsx"
